...
기존 스케일과 자세를 동시에 추정하여 생긴 정확도 저하 문제를 스케일과 포즈 예측을 분리하는 새로운 접근으로 극복함.  
노이즈에 취약한 PnP 기반 6-DoF 추정을 안정화하기 위해 keypoint 표현을 스케일 정규화 방식으로 개선함으로써 자세 추정의 신뢰도를 높임.  
Point cloud로 변환하는 과정 없이 RGB-D 입력만으로 직접 6-DoF grasp pose와 gripper open width를 예측해 효율성과 계산 비용 측면에서 우수함을 보임.  
단순 합성 데이터만으로도 실제 환경에 일반화 가능한 sim-to-real 성능을 입증, 로봇 조작 분야에서의 실제 활용 가능성을 크게 넓힘.  
주요 두 가지 기술적 개선점(스케일 분리 예측 및 스케일 정규화 keypoint)이 각각 성능 향상에 크게 기여했고, 이는 다른 6-DoF 파지 연구에도 시사점 제공.

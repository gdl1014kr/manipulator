...
KGNv2- GraspNet, KGN 개선 버전. Keypoint 기반 grasp pose(회전 및 방향) 추정. grasp pose의 scale(거리)은 별도의 네트워크를 통해 독립적으로 예측.   grasp pose 예측 정확도 향상 목표.  3차원 공간에서의 완전한 grasp pose 생성 목표
...
KGNv2의 핵심 아이디어:
키포인트 정확도에 대한 의존도를 낮추기 위해 새로운 네트워크 설계
키포인트로부터 파지 자세(포즈)를 추정하는 동시에 카메라까지의 거리(스케일)를 별도로 예측
키포인트 출력 공간을 재설계하여, 키포인트 위치 노이즈가 PnP 알고리즘(2D-3D 자세 추정 방법)에 미치는 영향을 줄임


성능: 기존 방법들보다 훨씬 좋은 성능을 보였고, 합성 데이터(가상 데이터)로만 학습했으나 실제 로봇 실험에서도 경쟁력 있는 결과를 나타내어 시뮬레이션에서 실제 환경으로의 전이(sim-to-real)가 가능함을 입증함.

Abstract (초록)
기존 키포인트 기반 방법의 문제점인 이미지 공간 키포인트 예측 정확도에 대한 높은 의존성을 줄이기 위해 새로운 네트워크 설계를 도입했습니다.
제안하는 KGNv2는 파지 자세(pose)와 카메라-파지 간 거리(scale)를 분리하여 추정합니다.
PnP 알고리즘 솔루션에 대한 키포인트 예측 노이즈의 영향을 완화하기 위해 키포인트 출력 공간을 재설계했습니다 (Scale-normalized keypoints).
실험 결과, 제안된 방법이 기준 모델(baseline)보다 훨씬 뛰어난 성능을 보이며, 특히 파지 성공률이 약 5% 향상되는 등 그 설계의 유효성을 입증했습니다.
단순한 합성 객체 데이터만으로 훈련했음에도 불구하고, 실제 로봇 실험에서 경쟁력 있는 결과를 보여 sim-to-real(시뮬레이션에서 실제 환경으로의 전이) 능력을 입증했습니다.
...
grasp pose 추정의 정확도를 높임. => Keypoint의 의존성 낮춤
...
기존 KGN 대비 grasp 성공률이 약 5% 향상되는 등 성능이 크게 개선됨.  
...
단순한 합성 기하학적 객체 데이터만으로 학습했음에도 불구하고, 실제 세계의 다양한 가정용 물체에 대해 경쟁력 있는 성능을 달성하여 강력한 sim-to-real 일반화 능력을 보였습니다.

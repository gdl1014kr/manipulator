...
기존 keypoint 기반 접근 방식의 한계(파지 자세의 스케일과 회전/위치를 동시에 추정하며 발생하는 불안정성과 정확도 저하)를 극복하기 위해 파지 자세 추정 문제를 스케일과 자세 예측으로 분리하고, PnP 알고리즘의 노이즈 민감성을 줄이기 위해 키포인트 표현 방식을 개선했다는 점, 기존 KGN의 단점을 보완하여 파지 자세 예측의 정확도 향상
